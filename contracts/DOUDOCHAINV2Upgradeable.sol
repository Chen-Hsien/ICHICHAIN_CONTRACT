    ...
    LightweightGuardsUpgradeable
{
    ...
    error InvalidConfig();
    error OnlyVrfCoordinator(address caller, address expectedCoordinator);
    ...
    function initialize(
        ...
        if (doudoPointsAddress == address(0) || vrfCoordinator_ == address(0)) {
    ...
    function getSubPrizesDetail(uint256 seriesID) external view returns (SubPrize[] memory) {
        return seriesSubPrizes[seriesID];
    }
    ...
    function mint(uint256 seriesID, uint16[] calldata luckyNumbers) external nonReentrant whenNotPaused {
        ...
        _mintTickets(seriesID, msg.sender, luckyNumbers, series.priceInPoints, true);
    }
    ...
    function adminMint(
        ...
        if (series.isRefund) revert SeriesIsRefund();
        if (quantity == 0 || quantity > series.remainingTicketNumbers) revert NotEnoughNFTsRemaining();
        _mintTickets(seriesID, to, luckyNumbers, 0, false);
    ...
    function setVrfConfig(
        ...
        if (vrfCoordinator_ == address(0) || callbackGasLimit_ == 0) revert InvalidConfig();
        vrfCoordinator = vrfCoordinator_;
        subscriptionId = subscriptionId_;
        keyHash = keyHash_;
    ...
    function rawFulfillRandomWords(uint256 requestId, uint256[] calldata randomWords) external {
        ...
        } else if (kind == RequestKind.RedrawMain) {
            _fulfillRedraw(requestId, randomWords[0]);
        } else if (kind == RequestKind.RedrawConsolation) {
            _fulfillRedraw(requestId, randomWords[0]);
        } else {
            revert InvalidConfig();
        }
    ...
    function _requestRandomWords(uint32 numWords) internal returns (uint256 requestId) {
        ...
        requestId = IVRFCoordinatorV2PlusMinimal(coordinator).requestRandomWords(
        ...
        requestCoordinator[requestId] = coordinator;
    }
    ...
    function _fulfillRedraw(uint256 requestId, uint256 randomWord) internal {
        uint256 seriesID = requestToSeries[requestId];
        address user = requestToRedrawUser[requestId];
        uint256 prizeId = _drawPrize(seriesID, randomWord);
        _mintOneTicketWithoutPoints(seriesID, user, prizeId, true);
    ...
    function _returnPrizeSlot(uint256 seriesID, uint256 subPrizeID) internal {
        ...
        for (uint256 i = 0; i < prizes.length; i++) {
            if (prizes[i].subPrizeID == subPrizeID) {
                ...
                emit UpdatePrize(seriesID, subPrizeID, prizes[i].subPrizeRemainingQuantity);
                return;
            }
        }
        revert InvalidSeriesInput();
