...
async function main() {
  ...
  const execute =
    process.env.EXECUTE_OPENING_DISCOUNT_ROUNDS_UPGRADE ===
  ...
  const reportPath = path.join(
    process.cwd(),
    "deployments",
    `opening-discount-rounds-${target.chainId}.json`,
  );
  ...
  if (target.chainId === 42161n) {
    ...
    checkpoint.transactions[
      "upgrade:DOUDO_BUNDLE_MODULE:openingDiscountRounds"
    ] = upgradeTxHash;
    checkpoint.completed = [
      ...new Set([
        ...checkpoint.completed,
        "upgrade:bundle:openingDiscountRounds",
      ]),
    ];
    await fs.writeFile(
      checkpointPath,
...
main().catch((error) => {
  console.error(
    `Opening discount rounds upgrade stopped at ${stage}; error type=${error?.name ?? "unknown"}, code=${error?.code ?? "none"}. Inspect the deployment checkpoint before retrying.`,
  );
  process.exitCode = 1;
});
